...
GDRIVE_API_KEY        = "AIzaSyBYhpyxe3LJjkIGksDm--egjxDwXBYhCu8"
...
def fetch_gdrive_file_list(folder_id, api_key):
    ...
    try:
        ...
        files = [
            ...
            or os.path.splitext(f.get("name", ""))[1].lower() in SUPPORTED_EXTS
        ]
        return files
...
def sync_gdrive_folder(folder_id, api_key, local_folder):
    """
    List the Drive folder and download any new images.
    ...
    for name, fid in remote.items():
        ...
        if not os.path.exists(local_path):
            ...
            download_gdrive_file(fid, api_key, local_path)
